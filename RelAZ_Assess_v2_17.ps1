...
If ($managementGroupId) {
    Write-Output "Resolving subscriptions under Management Group: $managementGroupId"
    $mgSubs = Search-AzGraph -Query "ResourceContainers | where type == 'microsoft.resources/subscriptions' | project subscriptionId" -ManagementGroup $managementGroupId -First 1000
    $mgSubIds = $mgSubs | Select-Object -ExpandProperty subscriptionId
    $sublist = $sublist | Where-Object { $_.Id -in $mgSubIds }
    Write-Output "$($sublist.Count) subscription(s) found under Management Group '$managementGroupId'"
}
